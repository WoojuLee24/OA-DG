...
class BBoxHead(BaseModule):
    ...
    def _get_target_single_with_absolute(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
                           pos_gt_labels, cfg):
        """Calculate the ground truth for proposals in the single image
        according to the sampling results.
        ...
        bbox_targets = pos_bboxes.new_zeros(num_samples, 4)
        bbox_weights = pos_bboxes.new_zeros(num_samples, 4)
        bbox_absolute_targets = pos_bboxes.new_zeros(num_samples, 4)
        if num_pos > 0:
            labels[:num_pos] = pos_gt_labels
            pos_weight = 1.0 if cfg.pos_weight <= 0 else cfg.pos_weight
            label_weights[:num_pos] = pos_weight
            ...
            bbox_targets[:num_pos, :] = pos_bbox_targets
            bbox_weights[:num_pos, :] = 1
            bbox_absolute_targets[:num_pos, :] = pos_gt_bboxes
        if num_neg > 0:
            label_weights[-num_neg:] = 1.0

        return labels, label_weights, bbox_targets, bbox_weights, bbox_absolute_targets
    ...
    def get_targets_with_absolute(self,
                    sampling_results,
                    gt_bboxes,
                    gt_labels,
                    rcnn_train_cfg,
        ...
        pos_gt_bboxes_list = [res.pos_gt_bboxes for res in sampling_results]
        pos_gt_labels_list = [res.pos_gt_labels for res in sampling_results]
        labels, label_weights, bbox_targets, bbox_weights, bbox_absolute_targets = multi_apply(
            self._get_target_single_with_absolute,
            pos_bboxes_list,
            neg_bboxes_list,
            pos_gt_bboxes_list,
            pos_gt_labels_list,
        ...
        if concat:
            ...
            bbox_targets = torch.cat(bbox_targets, 0)
            bbox_weights = torch.cat(bbox_weights, 0)
            bbox_absolute_targets = torch.cat(bbox_absolute_targets, 0)
        return labels, label_weights, bbox_targets, bbox_weights, bbox_absolute_targets
    ...
    def analysis_regions(self, rois, cls_score, bbox_pred, img_shape,
                         ...
                         img_metas=None, each_bbox=True, each_class=True):
        # some loss (Seesaw loss..) may have custom activation
        ...
        analysis_multiclass_nms(bboxes, scores, cfg.score_thr, cfg.nms, cfg.max_per_img,
                                img_metas=img_metas, each_bbox=each_bbox, each_class=each_class)
